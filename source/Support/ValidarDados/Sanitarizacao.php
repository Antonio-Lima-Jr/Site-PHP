...
namespace Source\Support\ValidarDados;
...
class Sanitarizacao
{
	protected $filter_var_enabled = false ; 

	public function __construct() {
		# verificando se a funcao filter_var do PHP 
		# esta disponivel para uso.
		# Essa função esta presente nas versos do PHP 
		# acima de 5.2.0. Mas para quem usa a Locaweb, 
		# é melhor verificar e dar uma alternativa.
		$this->filter_var_enabled = function_exists( 'filter_var' );
	}
    ...
    public function xssClean( $data )
    {
		...
		else $data = $this->xssCleanOldWay( $data );
     ...
     * @return Mixed
     */
    protected function xssCleanOldWay($data) {
    	if( is_array($data) ):
    		foreach($data as $k => $v):
     ...
     * @return type
     */
    public function tornarSeguro( $data = null )
    {
    ...
    public function texto( $param ) {
        return $this->tornarSeguro( $param );
    }
     ...
     * @return string
     */
    public function letras( $param = null ) {
        return is_null($param) ? ''
            : preg_replace('/[^\s\p{L}]/u','',$this->tornarSeguro($param) );
    }
     ...
     * @return string
     */
    public function textoSemEspacos( $param ) {
        $tmp = $this->tornarSeguro( $param );
     ...
     * @return int
     */
    public function numeros( $param = null ) {
        if( $param == null || empty($param) ) return null;
        $variable = $this->tornarSeguro( $param );
     ...
     */
    public function dinheiro($numero) {
    ...
    public function decimal( $valor = null, $precisao = null )
    {
        ...
        if( $ponto_pos === false && $virgu_pos === false 
            ...
            return $this->numeros($valor) . $precisao;
